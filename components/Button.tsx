...
export default function Button({
  ...
  return url ? (
    ...
    <button
      className={`${width} ${color} ${hoverColor} text-white font-medium py-4 px-6 ${rounded} tracking-wider  duration-150 active:scale-100 active:${color} uppercase font-work font-bold shadow-md  ${disabled ? 'opacity-70 cursor-default' : 'hover:shadow-lg hover:scale-105 cursor-pointer'
        }`}
    >
      {children}
    </button>
