            ...
            <li key={link.title}>
              <Link href={link.page}>
                <a
                  className={`
                  ${
                    router.pathname === link.page || link.page.includes(router.pathname.split('/')[1])
                      ? 'bg-accent-500 text-white'
                      : ''
                  }
                  py-[13px] px-[20px] hover:bg-accent-500 active:bg-accent-600 duration-100 hover:text-white rounded font-sans tracking-wide font-medium`}
                >
                  {link.title}
                </a>
              </Link>
