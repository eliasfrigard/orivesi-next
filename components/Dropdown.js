...
export default function Title({ title, items, active, scrollPosition }) {
  ...
  return (
    <div onMouseEnter={open} onMouseLeave={close}>
      ...
      {isOpen ? (
        <div
          className={`absolute z-50 flex flex-col gap-1 mt-2 translate-x-[-3.9rem] w-[220px] p-4 rounded-lg duration-300  ${
        ...
        >
          {items.map((item) => (
            <Link href={item.page} key={item.title}>
              <div
                className={`z-50 bg-secondary-500 hover:bg-accent-500 duration-100 rounded-lg py-[13px] px-[20px] font-sans tracking-wide font-medium text-primary-500 cursor-pointer border-4 border-primary-500 shadow-lg`}
              >
                {item.title}
              </div>
            </Link>
